"""Train LightGBM on train_feature data only with 5-fold CV."""
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(cv.split(x_train_full, y_train_full), start=1):
        ...
        model = lgb.LGBMClassifier(
            **_lgbm_params(scale_pos_weight),
            n_estimators=config.LGBM_NUM_BOOST_ROUND,
        )
        model.fit(
            x_train_encoded,
            y_train_fold,
            eval_set=[(x_valid_encoded, y_valid_fold)],
            eval_metric="auc",
        ...
        logger.info(
            "train_feature LightGBM CV fold=%s auc=%.6f pr_auc=%.6f f1=%.6f",
            fold_idx,
            metrics["auc_roc"],
            metrics["pr_auc"],
            metrics["f1_score"],
...
def _train_final_model(
    x_train_full: pd.DataFrame,
    y_train_full: pd.Series,
) -> tuple[lgb.LGBMClassifier, FoldEncoder, float]:
    """Train final LightGBM model on train_feature train split."""
...
def _save_model_bundle(
    ...
    """Save train_feature-only LightGBM model bundle."""
    ensure_directory(config.MODELS_SAVED_DIR)
    threshold_tag = f"{int(round(threshold * 100)):03d}"
    model_path = config.MODELS_SAVED_DIR / f"lgbm_train_feature_cv5_threshold{threshold_tag}_{trained_at}.pkl"
    payload = {
        "model": model,
        "encoder": encoder,
        "threshold": float(threshold),
        "version": "lgbm_train_feature_cv5",
...
def _save_precision_recall_chart(y_true: pd.Series, y_score: np.ndarray, target_path: Path) -> None:
    ...
    plt.plot(recall, precision, color="darkgreen", linewidth=2)
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.title("LightGBM Train-Feature Precision-Recall Curve")
...
def _write_report(
    ...
    """Write train_feature-only LightGBM metrics report."""
    ensure_directory(config.REPORTS_DIR)
    report_path = config.REPORTS_DIR / "metrics_lgbm_train_feature_cv5.md"
    lines = [
...
def train_lgbm_train_feature_cv5(threshold: float = 0.30) -> dict[str, Any]:
    """Train LightGBM using train_feature data only, with CV5 and fixed threshold."""
    ...
    pr_chart_path = config.REPORTS_DIR / "lgbm_train_feature_precision_recall_curve.png"
